    ...
    unsigned int last)
{
  ...
  for (unsigned int i = first; i <= last; i++) {
    ...
    // if (retval != ERROR_OK)
    //   goto flash_lock;
    // LOG_INFO("py32c643x flash erase at 0x%08x", (uint32_t)(bank->base + bank->sectors[i].offset));
...
#include "../../../contrib/loaders/flash/py32/py32c643.inc"
	};
    ...
    uint32_t offset, uint32_t count)
{
  ...
  LOG_DEBUG("py32c643x flash write success");
...
static int py32x_get_property_addr(struct target *target, struct py32x_property_addr *addr)
{
  ...
  case CORTEX_M0P_PARTNO: /* py32c643 devices */
...
COMMAND_HANDLER(py32x_handle_options_write_command)
{
  ...
  command_print(CMD, "py32x write options complete.\n"
        ...
        "for the new settings to take effect.",
        "'py32c643 options_load' command or ");
...
static const struct command_registration py32c643_exec_command_handlers[] = {
  {
    .name = "lock",
...
static const struct command_registration py32c643_command_handlers[] = {
  {
    .name = "py32c643",
    .mode = COMMAND_ANY,
    .help = "py32c643 flash command group",
    .usage = "",
    .chain = py32c643_exec_command_handlers,
  },
  COMMAND_REGISTRATION_DONE
};

const struct flash_driver py32c643_flash = {
  .name = "py32c643",
  .commands = py32c643_command_handlers,
