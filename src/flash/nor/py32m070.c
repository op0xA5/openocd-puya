...
#define OPTR_DEFAULT        0x0000D8AA
#define SDKR_DEFAULT        0x0000001F
#define WRPR_DEFAULT        0x0000FFFF
    ...
    unsigned int last)
{
  ...
  for (unsigned int i = first; i <= last; i++) {
    ...
    // if (retval != ERROR_OK)
    //   goto flash_lock;
    // LOG_INFO("py32m070x flash erase at 0x%08x", (uint32_t)(bank->base + bank->sectors[i].offset));
...
#include "../../../contrib/loaders/flash/py32/py32m070.inc"
	};
    ...
    uint32_t offset, uint32_t count)
{
  ...
  LOG_DEBUG("py32m070x flash write success");
...
static int py32x_get_property_addr(struct target *target, struct py32x_property_addr *addr)
{
  ...
  case CORTEX_M0P_PARTNO: /* py32m070 devices */
    addr->device_id = 0x40015800;
    addr->flash_size = 0x1FFF33F8;
    return ERROR_OK;
    /* fallthrough */
  default:
...
static int py32x_probe(struct flash_bank *bank)
{
  ...
  /* get flash size from target. */
	...
	// case 0x06188061: /* pt063 */
		...
		flash_size_in_kb = (((flash_size_in_kb & 0x03) >> 0) + 1) * 32;
	// 	break;
	// default:
...
COMMAND_HANDLER(py32x_handle_options_write_command)
{
  ...
  command_print(CMD, "py32x write options complete.\n"
        ...
        "for the new settings to take effect.",
        "'py32m070 options_load' command or ");
...
static const struct command_registration py32m070_exec_command_handlers[] = {
  {
    .name = "lock",
...
static const struct command_registration py32m070_command_handlers[] = {
  {
    .name = "py32m070",
    .mode = COMMAND_ANY,
    .help = "py32m070 flash command group",
    .usage = "",
    .chain = py32m070_exec_command_handlers,
  },
  COMMAND_REGISTRATION_DONE
};

const struct flash_driver py32m070_flash = {
  .name = "py32m070",
  .commands = py32m070_command_handlers,
