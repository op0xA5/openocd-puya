...
#define OPTR_DEFAULT        0x6F5590AA
#define SDKR_DEFAULT        0xFFF0000F
#define BTCR_DEFAULT        0x7FFF8000
#define WRPR_DEFAULT        0xFF0000FF
    ...
    unsigned int last)
{
  ...
  for (unsigned int i = first; i <= last; i++) {
    ...
    // if (retval != ERROR_OK)
    //   goto flash_lock;
    // LOG_INFO("py32t020x flash erase at 0x%08x", (uint32_t)(bank->base + bank->sectors[i].offset));
...
#include "../../../contrib/loaders/flash/py32/py32t020.inc"
	};
    ...
    uint32_t offset, uint32_t count)
{
  ...
  LOG_DEBUG("py32t020x flash write success");
...
static int py32x_get_property_addr(struct target *target, struct py32x_property_addr *addr)
{
  ...
  case CORTEX_M0P_PARTNO: /* py32t020 devices */
...
COMMAND_HANDLER(py32x_handle_options_write_command)
{
  ...
  while (CMD_ARGC) {
    if (strcmp("OPTR", CMD_ARGV[0]) == 0) {
      ...
      CMD_ARGC--;
      CMD_ARGV++;
    } else if (strcmp("BTCR", CMD_ARGV[0]) == 0) {
      if (CMD_ARGC < 2)
        return ERROR_COMMAND_SYNTAX_ERROR;
      COMMAND_PARSE_NUMBER(u32, CMD_ARGV[1], btcr);
      CMD_ARGC--;
      CMD_ARGV++;
  ...
  command_print(CMD, "py32x write options complete.\n"
        ...
        "for the new settings to take effect.",
        "'py32t020 options_load' command or ");
...
static const struct command_registration py32t020_exec_command_handlers[] = {
  {
    .name = "lock",
...
static const struct command_registration py32t020_command_handlers[] = {
  {
    .name = "py32t020",
    .mode = COMMAND_ANY,
    .help = "py32t020 flash command group",
    .usage = "",
    .chain = py32t020_exec_command_handlers,
  },
  COMMAND_REGISTRATION_DONE
};

const struct flash_driver py32t020_flash = {
  .name = "py32t020",
  .commands = py32t020_command_handlers,
