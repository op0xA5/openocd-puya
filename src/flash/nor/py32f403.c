...
// #define PY32_OB_RDP         0x1FFF3100
// #define PY32_OB_USER        0x1FFF3101
// #define PY32_OB_SDK_STRT    0x1FFF3108
// #define PY32_OB_SDK_END     0x1FFF3109
// #define PY32_OB_WRP0        0x1FFF3118
// #define PY32_OB_WRP1        0x1FFF3119
...
#define OPTR_DEFAULT        0x000070AA
// #define SDKR_DEFAULT        0x0000001F
#define WRPR_DEFAULT        0x00000FFF
...
FLASH_BANK_COMMAND_HANDLER(py32x_flash_bank_command)
{
  ...
  /* The flash write must be aligned to a 256B boundary */
  bank->write_start_alignment = bank->write_end_alignment = 256;
    ...
    unsigned int last)
{
  ...
  for (unsigned int i = first; i <= last; i++) {
    ...
    // if (retval != ERROR_OK)
    //   goto flash_lock;
    // LOG_INFO("py32f403x flash erase at 0x%08x", (uint32_t)(bank->base + bank->sectors[i].offset));
...
#include "../../../contrib/loaders/flash/py32/py32f403.inc"
	};
    ...
    uint32_t offset, uint32_t count)
{
   ...
   * The flash infrastructure ensures it, do just a security check
   */
  assert(offset % 0x100 == 0);
  assert(count % 0x100 == 0);

  if (offset & 0xFF) {
    LOG_ERROR("offset 0x%" PRIx32 " breaks required 256-bytes alignment", offset);
    return ERROR_FLASH_DST_BREAKS_ALIGNMENT;
  }
  
  if(count & 0xFF){
        LOG_ERROR("size 0x%" PRIx32 " breaks required 256-bytes alignment", count);
        return ERROR_FLASH_DST_BREAKS_ALIGNMENT;
    }
  ...
  while (count) {
    // flash memory 256-bytes program        
    retval = target_write_u32(target, PY32_FLASH_CR, FLASH_PG);
    if (retval != ERROR_OK)
        goto reset_pg_and_lock;

    retval = target_write_buffer(target, (uint32_t)(bank->base + addr), 4*(0x40-1), &buffer[addr-offset+0]);
    if (retval != ERROR_OK)
      goto reset_pg_and_lock;
    addr += 4*(0x40-1);
    ...
    if (0 == ((addr - 0x100)%0x400)) {
      LOG_INFO("Programming addr 0x0%" PRIx32 " success, done %d%%.", (uint32_t)(bank->base + addr - 0x100), (total - count) * 100 / total);
    }
  }

  LOG_DEBUG("py32f403x flash write success");
...
static int py32x_get_property_addr(struct target *target, struct py32x_property_addr *addr)
{
  ...
  case CORTEX_M4_PARTNO: /* py32f403 devices */
    addr->device_id = 0x40015800;
    addr->flash_size = 0x1FFF5700;
    return ERROR_OK;
    /* fallthrough */
  default:
...
static int py32x_probe(struct flash_bank *bank)
{
  ...
  /* get flash size from target. */
	...
	// case 0x06188061: /* pt063 */
		sector_size = 0x800;
		py32x_info->psector_size = 32/2;//block_size/sector_size
		max_flash_size_in_kb = 384;

		flash_size_in_kb = (((flash_size_in_kb & 0x1F) >> 0) + 1) * 16;

    flash_size_in_kb = min(flash_size_in_kb, max_flash_size_in_kb);
	// 	break;
	// default:
...
COMMAND_HANDLER(py32x_handle_options_write_command)
{
  ...
  command_print(CMD, "py32x write options complete.\n"
        ...
        "for the new settings to take effect.",
        "'py32f403 options_load' command or ");
...
static int py32x_init(struct target *target)
{
  ...
  retval = target_read_u32(target, 0x1FFF3220, &hsi_trim);
  if (retval != ERROR_OK)
    return retval;
  ...
  retval = target_read_u32(target, 0x1FFF32D8, &flash_trim);
  if (retval != ERROR_OK)
    return retval;
  ...
  retval = target_read_u32(target, 0x1FFF32E0, &flash_trim);
  if (retval != ERROR_OK)
    return retval;
  ...
  retval = target_read_u32(target, 0x1FFF32E8, &flash_trim);
  if (retval != ERROR_OK)
    return retval;
  ...
  retval = target_read_u32(target, 0x1FFF32F0, &flash_trim);
  if (retval != ERROR_OK)
    return retval;
  ...
  retval = target_read_u32(target, 0x1FFF32F8, &flash_trim);
  if (retval != ERROR_OK)
    return retval;
...
static const struct command_registration py32f403_exec_command_handlers[] = {
  {
    .name = "lock",
...
static const struct command_registration py32f403_command_handlers[] = {
  {
    .name = "py32f403",
    .mode = COMMAND_ANY,
    .help = "py32f403 flash command group",
    .usage = "",
    .chain = py32f403_exec_command_handlers,
  },
  COMMAND_REGISTRATION_DONE
};

const struct flash_driver py32f403_flash = {
  .name = "py32f403",
  .commands = py32f403_command_handlers,
