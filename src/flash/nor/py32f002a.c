...
#define PY32_OB_RDP         0x1FFF0E80
#define PY32_OB_USER        0x1FFF0E81
#define PY32_OB_SDK_STRT    0x1FFF0E84
#define PY32_OB_SDK_END     0x1FFF0E85
#define PY32_OB_WRP0        0x1FFF0E8C
#define PY32_OB_WRP1        0x1FFF0E8D
...
#define OPTR_DEFAULT        0x0000BEAA
    ...
    unsigned int last)
{
  ...
  for (unsigned int i = first; i <= last; i++) {
    ...
    // if (retval != ERROR_OK)
    //   goto flash_lock;
    // LOG_INFO("py32f002ax flash erase at 0x%08x", (uint32_t)(bank->base + bank->sectors[i].offset));
...
#include "../../../contrib/loaders/flash/py32/py32f002a.inc"
	};
    ...
    uint32_t offset, uint32_t count)
{
  ...
  LOG_DEBUG("py32f002ax flash write success");
...
static int py32x_get_property_addr(struct target *target, struct py32x_property_addr *addr)
{
  ...
  case CORTEX_M0P_PARTNO: /* py32f002a devices */
    addr->device_id = 0x40015800;
    addr->flash_size = 0x1FFF0FFC;
    return ERROR_OK;
    /* fallthrough */
  default:
...
static int py32x_probe(struct flash_bank *bank)
{
  ...
  /* get flash size from target. */
	...
	// case 0x60001000: /* pt061 */
		sector_size = 0x1000;
		py32x_info->psector_size = 1;
		max_flash_size_in_kb = 64;

		flash_size_in_kb = (((flash_size_in_kb & 0x07) >> 0) + 1) * 8;
	// 	break;
	// default:
...
COMMAND_HANDLER(py32x_handle_options_write_command)
{
  ...
  command_print(CMD, "py32x write options complete.\n"
        ...
        "for the new settings to take effect.",
        "'py32f002a options_load' command or ");
...
static const struct command_registration py32f002a_exec_command_handlers[] = {
  {
    .name = "lock",
...
static const struct command_registration py32f002a_command_handlers[] = {
  {
    .name = "py32f002a",
    .mode = COMMAND_ANY,
    .help = "py32f002a flash command group",
    .usage = "",
    .chain = py32f002a_exec_command_handlers,
  },
  COMMAND_REGISTRATION_DONE
};

const struct flash_driver py32f002a_flash = {
  .name = "py32f002a",
  .commands = py32f002a_command_handlers,
